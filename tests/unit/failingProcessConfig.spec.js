...
describe('Failing http config tests', () => {
    ...
    it('should fail if the environment variable does not exist', () => {
        let entersException = false;
        try {
            processConfigFactory.create({
                framework: {
            ...
            expect(e.message).to.be.equal(`Invalid config. Environment variable 'env' does not exist`)
        }
