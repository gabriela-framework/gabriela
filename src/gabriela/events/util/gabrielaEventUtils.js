...
async function runOnTestCleanup(events, rootCompiler, err) {
    if (!hasKey(events, GABRIELA_EVENTS.ON_TEST_CLEANUP)) return;
     ...
     */
    try {
        callSingleGabrielaEvent.call(this, events[GABRIELA_EVENTS.ON_TEST_CLEANUP], rootCompiler, err);
    } catch (eventError) {
        eventError.message = `An error has been thrown in '${GABRIELA_EVENTS.ON_TEST_CLEANUP}' gabriela event with message: '${eventError.message}'. This is regarded as an unrecoverable error and the server has closed`;

        if (events[GABRIELA_EVENTS.ON_CATCH_ERROR]) {
            return callSingleGabrielaEvent.call(this, events[GABRIELA_EVENTS.ON_TEST_CLEANUP], rootCompiler, eventError);
        }
